...
class AddNewPrebidPartnerTests(TestCase):
  ...
  @patch('settings.DFP_CREATIVE_FORMAT', '{bidder_code}: HB {order_name}, #{num:0>2}', create=True)
  @patch('tasks.add_new_prebid_partner.setup_partner')
  @patch('tasks.add_new_prebid_partner.input', return_value='y')
  def test_custom_creative_format(self, mock_input, mock_setup_partners,
    mock_dfp_client):
    """
    Ensure we use the currency code setting if it exists.
    """
    tasks.add_new_prebid_partner.main()
    args, kwargs = mock_setup_partners.call_args
    self.assertEqual(args[11], '{bidder_code}: HB {order_name}, #{num:0>2}')
  ...
  def test_custom_enable_safeframe(self, mock_input, mock_setup_partners,
    ...
    tasks.add_new_prebid_partner.main()
    args, kwargs = mock_setup_partners.call_args
    self.assertEqual(args[12], False)
  ...
  def test_setup_partner(self, mock_get_users, mock_get_placements,
    ...
    tasks.add_new_prebid_partner.setup_partner(user_email=email, advertiser_name=advertiser, order_name=order,
                                               ...
                                               currency_code='USD', line_item_format=u'{bidder_code}: HB ${price:0>5}', creative_format='{bidder_code}: HB {order_name}, #{num:0>2}', enable_safeframe=True)
    ...
    (mock_create_creatives.create_duplicate_creative_configs
      .assert_called_once_with(bidder_code, order, 246810, 2, '{bidder_code}: HB {order_name}, #{num:0>2}', True))
